...
class Agent(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        isasync = bool(int(query.get('async', [False])[0]))
        output = run_command(command, asynchronous=isasync)
...
def main():
    ...
    logging.basicConfig(level=True and 10 or 20)

    logging.info("Serving requests at %s %d.", "0.0.0.0", 8080)
