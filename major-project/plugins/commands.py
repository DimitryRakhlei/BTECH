...
class CommandsHook(Hook):
    ...
    * Event("run_sample", async=True, sample="/local/path/file.exe",
    ...
    def run_command_handler(self, event):
        ...
        isasync = hasattr(event, 'async') and event.isasync or False
        response = self.command_request(event.command, isasync)

        self.log_command_response(event.command, response, isasync)

    def run_sample_handler(self, event):
        try:
            isasync = hasattr(event, 'async') and event.isasync or False
            response = self.sample_request(event.command, event.sample, isasync)
    ...
    def command_request(self, command, isasync):
    ...
    def sample_request(self, command, sample, isasync):
        url = 'http://%s:%d' % (self.host, self.port)
    ...
    def log_command_response(self, command, response, isasync):
        if not isasync:
