...
class NetworkTracerHook(Hook):
    ...
    def start_trace_handler(self, event):
        ...
        # patch from dimitryrakhlei
        # need to touch and chmod the file before tshark can use it in user directory
        from subprocess import Popen, PIPE
        process = Popen([ "touch", self.pcap_path ], stdout=PIPE, stderr=PIPE)
        print(process.communicate())
        process = Popen([ "chmod", "o=rw", self.pcap_path ], stdout=PIPE, stderr=PIPE)
        print(process.communicate())
        # endpatch
